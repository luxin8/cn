...
当前覆盖国内 **华北-北京（三可用区）、华南-广州（三可用区）、华东-宿迁（单可用区）、华东-上海 （三可用区）** 四地域多可用区，不同地域之间完全隔离，以保证不同地域间最大程度的稳定性和容错性，地域信息详情请参见[地域与可用区]。
...
* 线路：当前各地域中心可用区仅提供BGP IP，相较于非BGP（单线）IP，BGP IP提供更稳定的访问速度。
